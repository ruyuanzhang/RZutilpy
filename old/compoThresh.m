function compoThresh(subjInitials,p,test_con)
% function compoThresh(subjInitials,p,test_con)
...
%       <orientation>: orientation of grating
%       <speed>: speed of stimuli
%       <nTrials>: how many trials per staircase
...
ListenChar(2);  % you need to test the keyboard number here

% --------------- Monitor Parameters -----------------
viewDist = 75; % view distance, 75 cm 
monitorSize = [40 40]; % width, heigh in cm
...
radius = p.radius;
contrast = p.contrast;
if train_condition==1
    trained_direction='left/right';
else
    trained_direction='up/down';
end
...
%nTrials                   = 80;      %number of nTrials per staircase
n_alternatives           = 2;        
...
staircase_list =sort(repmat((1:size(up_down_design,1))',nTrials,1));
direction_list =repmat((1:n_alternatives)',total_trials/n_alternatives,1);  
...
data(:,2)=staircase_list;data(:,6)=direction_list;
    ...
    capturewindow = [919         359        1640        1080];
    ...
    while trial <= total_trials
        ...
        for i = 1:ceil(mv_length/3)
            ...
            Screen('Flip',w);            
%             %demo capture
%             im = Screen('GetImage',w,capturewindow,'frontBuffer',[],[]);
%             imname = strcat(num2str(abc),'.png');
...
%         
        ...
        %duration
        %%%%------update staircase here---------%%%%%%%%
%         up_down_design = [ 1,2; 2,1; 5,1 ];
%         up_down_step_in_log = 0.15;    
%         curr_up_downs = zeros( length( curr_stair_t), 2 );
    ...
    filename = strcat(subjInitials,'_compo_',num2str(test_con),'.mat');
