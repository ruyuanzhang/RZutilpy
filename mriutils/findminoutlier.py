def findminoutlier(funcfiles, outputDir=None):
    ...
    nRuns = len(funcfiles)
    nVols_list = [int(unix_wrapper(f'3dinfo -nt {i}', verbose=0, wantreturn=True)) for i in funcfiles]
    ...
    pressfile.close()
    # combine all files
    unix_wrapper(f'cat {outputDir}/outcount.r*.1D > {outputDir}/outcount_rall.1D')
    ...
    unix_wrapper(f'3dbucket -prefix {outputDir}/vr_base_min_outlier "{funcfiles[minoutrun-1]}[{minouttr}]" ')
